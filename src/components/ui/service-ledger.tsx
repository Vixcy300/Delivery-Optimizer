...
export function ServiceLedger({
  ...
  React.useEffect(() => {
    ...
    const compute = () => {
      raf = null;
      const stripBottom = tabsRef.current?.getBoundingClientRect().bottom ?? 0;
      const referenceY =
        stripBottom > 0 ? stripBottom + 20 : FALLBACK_REFERENCE_Y;
  ...
  /* Keep the active tab visible inside the horizontal scroller on mobile. */
  React.useEffect(() => {
    ...
    if (tab) {
      ...
      // Center the active tab in the scroller container
        ...
        <div className="mx-auto mt-10 flex max-w-3xl flex-col space-y-14 md:mt-16 md:space-y-24">
          {entries.map((entry, index) => (
            <div
              ...
              style={{
                scrollMarginTop: "calc(var(--ledger-sticky-top, 0px) + 5rem)",
              }}
            ...
            >
              ...
              {entry.video ? (
                ...
                <img
                  ...
                  className="mt-6 w-full rounded-lg object-cover ring-1 ring-border/60 md:mt-8"
                />
              ) : null}
